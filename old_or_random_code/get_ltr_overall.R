...
get_ltr_overall <- function(outcome_name, sex) {
  ...
  lt$age_name <- as.character(lt$age_name)
  ...
  pop$age_name <- as.character(pop$age_name)
  ...
  gbdfin$age_name <- as.character(gbdfin$age_name)
  ...
  tab <- data.frame(age_name = temp$age_name, # age group name
                    ...
                    m = temp$acm - temp$outcome_deaths/temp$population, # outcome-deleted death rate (per resident)
  ...
  for (i in 1:nrow(tab)) {
    if (i >= 2) {
      tab$l[i] <- tab$l[i-1] * exp(-5*(tab$m[i-1] + tab$r[i-1]))
    }
  }
  tab$a <- tab$l * (1 - exp(-5*(tab$m + tab$r))) * (tab$r / (tab$m + tab$r))
  
  tab$d <- tab$l * (1 - exp(-5*(tab$m + tab$r))) * (tab$m / (tab$m + tab$r))
  
  # Calculate a and d for last open ended age interval
  ind <- nrow(tab)
  tab$a[ind] <- tab$l[ind] * (tab$r[ind]/(tab$r[ind] + tab$m[ind]))
  tab$d[ind] <- tab$l[ind] * (tab$m[ind]/(tab$r[ind] + tab$m[ind]))
